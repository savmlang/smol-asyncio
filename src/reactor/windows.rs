...
impl Registration {
  ...
  pub(crate) fn add(&self, poller: &Poller, token: usize) -> Result<()> {
    ...
    unsafe {
      match self {
        ...
        Self::Handle(handle) => poller.add_waitable(*handle, Event::none(token), PollMode::Oneshot),
      }
    }
  }
  ...
  pub(crate) fn modify(&self, poller: &Poller, interest: Event) -> Result<()> {
    ...
    match self {
      Self::Socket(raw) => poller.modify(unsafe { BorrowedSocket::borrow_raw(*raw) }, interest),
